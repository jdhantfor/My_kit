  ...
  static Future<void> initializeDatabase() async {
    ...
    await db.execute('ALTER TABLE reminders_table ADD COLUMN isCompleted INTEGER');
        ...
        if (oldVersion < 17) {
    await db.execute('ALTER TABLE action_table ADD COLUMN quantity TEXT');
    await db.execute('ALTER TABLE action_table ADD COLUMN scheduleType TEXT');
    await db.execute('ALTER TABLE action_table ADD COLUMN notification TEXT');
    await db.execute('ALTER TABLE action_table ADD COLUMN isCompleted INTEGER');
        }
      },
      version: 17, 
    );
  ...
  ''');
    ...
    final reminders = await db.query(
      'reminders_table',
      where: 'user_id = ? AND type IN ("tablet", "action") AND date(startDate) <= ? AND (date(endDate) >= ? OR isLifelong = 1)',
    ...
    Map<String, List<Map<String, dynamic>>> groupRemindersByTime(List<Map<String, dynamic>> reminders) {
    ...
    final Map<String, dynamic> reminderToInsert = {
      ...
      'type': reminder['type'] ?? 'tablet', // Добавляем поле type с значением по умолчанию
    ...
    static Future<List<Map<String, dynamic>>> getPulseDataForPeriod(String userId, String startDate, String endDate, String period) async {
  final db = database;
  ...
  print('Raw pulse data for userId $userId, period $period, $startDate to $endDate: $result'); // Отладка сырых данных

  List<Map<String, dynamic>> data = [];
  if (period == 'Дни') {
    for (int i = 0; i < intervals; i++) {
      final hour = i.toString().padLeft(2, '0');
      final entry = result.firstWhere((e) => e['period'] == hour, orElse: () => {'period': hour, 'min_value': 0, 'max_value': 0});
      data.add({
        'label': '$hour:00',
    ...
    for (int i = 0; i < intervals; i++) {
      ...
      final entry = result.firstWhere((e) => e['period'] == dateStr, orElse: () => {'period': dateStr, 'min_value': 0, 'max_value': 0});
      data.add({
    ...
    for (int i = 1; i <= intervals; i++) {
      ...
      final entry = result.firstWhere((e) => e['period'] == day.padLeft(2, '0'), orElse: () => {'period': day, 'min_value': 0, 'max_value': 0});
      data.add({
        'label': i % 7 == 0 || i == 1 ? '$i' : '',
        'min': entry['min_value'] ?? 0,
        'max': entry['max_value'] ?? 0,
  ...
  print('Processed graph data for $period: $data'); // Отладка обработанных данных
  return data;
}

  static Future<Map<String, dynamic>> getPulseSummary(String userId, String startDate, String endDate) async {
    final db = database;
    final result = await db.rawQuery('''
  ...
  static Future<List<Map<String, dynamic>>> getPulseComments(String userId, String startDate, String endDate) async {
    final db = database;
    final result = await db.query(
      'pulse_data',
...
static Future<List<Map<String, dynamic>>> getUnattachedReminders(String userId) async {
  final db = database;
  return await db.query(
    'reminders_table',
...
static Future<List<Map<String, dynamic>>> getUnattachedActions(String userId) async {
  final db = database;
  return await db.query(
    'actions_table',
    where: '(courseid IS NULL OR courseid = -1) AND user_id = ?',
...
static Future<List<Map<String, dynamic>>> getUnattachedMeasurements(String userId) async {
  final db = database;
  return await db.query(
    'measurements_table',
    where: '(courseid IS NULL OR courseid = -1) AND user_id = ?',
