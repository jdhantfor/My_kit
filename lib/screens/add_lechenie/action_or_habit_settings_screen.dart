...
class ActionOrHabitSettingsScreenState
  ...
  void initState() {
    ...
    if (widget.initialData != null) {
      ...
      _isLifelong = data['isLifelong'] == 1;
      _startDate = data['startDate'] != null
          ? DateTime.parse(data['startDate'])
          : DateTime.now();
      _durationValue = int.tryParse(data['duration']?.toString() ?? '30') ?? 30;
      _durationUnit = data['durationUnit'] ?? 'дней';
      if (data['times'] != null) {
        _times.addAll(
            (data['times'] as List).map((t) => {'time': t['time']}).toList());
      } else if (data['selectTime'] != null) {
        _times.add({'time': data['selectTime']});
      }
      _selectedMealTime = data['mealTime'] ?? 'Выбор';
      _selectedScheduleType = data['schedule_type'] ?? 'daily';
      _intervalValue =
          int.tryParse(data['interval_value']?.toString() ?? '3') ?? 3;
      _selectedDaysMask =
          int.tryParse(data['selected_days_mask']?.toString() ?? '0') ?? 0;
      _durationValueForSchedule =
          int.tryParse(data['cycle_duration']?.toString() ?? '7') ?? 7;
      _breakValue = int.tryParse(data['cycle_break']?.toString() ?? '7') ?? 7;
  ...
  void _addActionOrHabit() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Действие добавлено')),
      );
