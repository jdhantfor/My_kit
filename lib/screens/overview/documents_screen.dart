...
class _FilesScreenState extends State<FilesScreen> {
  ...
  Future<void> _fetchFiles() async {
    try {
      print('Получаю файлы для ${widget.userId}/${widget.category}');
      var response = await http.get(
        Uri.parse(
            '${widget.serverUrl}/list/${widget.userId}/${widget.category}'),
      );
      print('Статус ответа: ${response.statusCode}');

      if (response.statusCode == 200) {
        var data = json.decode(response.body);
        print('Список файлов: ${data['files']}');
        setState(() {
          files = List<String>.from(data['files']);
        });
      } else {
        print('Ошибка получения списка: ${response.statusCode}');
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _downloadAndOpenFile(String fileName) async {
    try {
      String fileUrl =
          '${widget.serverUrl}/download/${widget.userId}/${widget.category}/$fileName';
      print('Скачиваю файл: $fileUrl');
      var response = await http.get(
        Uri.parse(fileUrl),
      );
      ...
      if (response.statusCode == 200) {
        ...
        print('Файл сохранён: $tempPath/$fileName');
      ...
      print('Исключение при скачивании: $e');
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Произошла ошибка: $e')),
      );
    }
