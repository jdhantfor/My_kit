  ...
  List<Map<String, dynamic>> _prepareWeeklyData(
    ...
    for (int i = 0; i < 7; i++) {
      ...
      if (dayData.isNotEmpty) {
        final avgSystolic =
            dayData.map((d) => d['systolic'] as num).reduce((a, b) => a + b) /
                dayData.length;
        final avgDiastolic =
            dayData.map((d) => d['diastolic'] as num).reduce((a, b) => a + b) /
                dayData.length;
        weeklyData.add({
          'date': date,
          'systolic': avgSystolic.clamp(0, 200), // Ограничиваем значение до 200
          'diastolic':
              avgDiastolic.clamp(0, 200), // Ограничиваем значение до 200
