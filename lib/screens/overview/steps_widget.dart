...
class StepsWidgetState extends State<StepsWidget> {
  ...
  List<Map<String, dynamic>> _prepareWeeklyData(
    ...
    for (int i = 0; i < 7; i++) {
      ...
      if (dayData.isNotEmpty) {
        // Вычисляем среднее количество шагов за день
        final total =
            dayData.map((d) => d['count'] as num).reduce((a, b) => a + b);
        avgCount = total / dayData.length;
        avgCount = avgCount.clamp(0, 20000); // Ограничиваем значение до 20000
      }
      weeklyData.add({
        'date': date,
        'count': avgCount,
  ...
  Widget build(BuildContext context) {
    const maxSteps = 20000.0; // Уже установлено 20000
                      ...
                      child: Row(
                        ...
                        children: stepsData.map((data) {
                          final double columnHeight = data['count'] != null
                              ? (maxHeight - 20) *
                                  (data['count'] / maxSteps).clamp(0.0, 1.0)
                              : 10.0;
                          return SizedBox(
                            width: 40,
                            child: Column(
