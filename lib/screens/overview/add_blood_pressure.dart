...
import 'package:flutter/services.dart'; // Добавляем для TextInputFormatter
...
class _AddBloodPressureState extends State<AddBloodPressure> {
  ...
  void _saveBloodPressure() async {
    ...
    if (systolic == null || systolic < _minValue || systolic > _maxValue) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('Систолическое давление должно быть от 1 до 200')),
      );
      return;
    }
    if (diastolic == null || diastolic < _minValue || diastolic > _maxValue) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('Диастолическое давление должно быть от 1 до 200')),
      );
      return;
    }
    if (pulse != null && (pulse < _minValue || pulse > _maxValue)) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Пульс должен быть от 1 до 200')),
      );
      return;
    }
