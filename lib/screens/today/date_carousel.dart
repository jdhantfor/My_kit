          ...
          children: [
            Text(
              DateFormat('EEE', 'ru').format(
                  date), // Убираем toUpperCase(), чтобы текст был маленькими буквами
              style: _getDayNameStyle(date, today, dayStatus),
            ),
            const SizedBox(height: 8.0), // Увеличиваем расстояние до 8 пикселей
            Container(
              width: 48,
              height: 48,
    ...
    if (isSelected) {
      if (status != null) {
        switch (status) {
          ...
          case DayStatus.none:
            style =
                style.copyWith(color: AppColors.primaryBlue); // Синий, как фон
            break;
        }
      } else {
        style = style.copyWith(color: AppColors.primaryBlue); // Синий, как фон
      }
    } else if (status != null) {
